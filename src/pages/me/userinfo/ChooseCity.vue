...
const list = ref([
  ...
  '海口', '三亚', '三沙', '儋州',
  '成都', '绵阳', '自贡', '攀枝花', '泸州', '德阳', '广元', '遂宁', '内江', '乐山', '资阳', '宜宾', '南充', '达州', '雅安', '广安', '巴中', '眉山',
  '贵阳', '六盘水', '遵义', '安顺', '毕节', '铜仁',
...
async function save(city: string) {
  _showLoading()
  try {
    await updateProfile({ province, city })
    store.setUserinfo({ province, city })
  } catch { /* ignore */ }
  _hideLoading()
  history.go(-2)
}
...
.choose-location {
  ...
  .row {
    ...
    height: 50rem; font-size: 14rem;
    transition: all 0.1s; background: var(--main-bg);

    .right {
      display: flex; justify-content: space-between;
      align-items: center; color: var(--second-text-color);
      img { margin-left: 10rem; width: 15px; }
    }
