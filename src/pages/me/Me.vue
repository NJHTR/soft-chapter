      ...
      <SlideItem>
        ...
        >
          <div ref="desc" class="desc">
            <!-- 未登录: 干净的登录引导 -->
            <div v-if="!isLoggedIn" class="guest-area" @click="$router.push('/login')">
              <div class="guest-header">
                <div class="guest-avatar">
                  <Icon icon="mdi:account-circle" />
                </div>
                <p class="guest-title">登录/注册</p>
                <p class="guest-sub">登录后查看更多精彩内容</p>
              </div>
...
export default {
  ...
  mounted() {
    nextTick(() => {
      ...
      this.refs.headerHeight = this.$refs.header ? this.$refs.header.offsetHeight : 0
