    ...
    <div class="list-container">
      ...
      <div v-show="currentTab === 'video'" class="tab-content">
        ...
        <div v-for="item in videoList" :key="'v-' + item.id" class="review-card">
          ...
          <div class="card-center">
            ...
            <div class="card-author" v-if="item.author">作者: {{ item.author.nickname || item.author.email }}</div>
          </div>
          <div class="card-right">
            <button class="btn-approve" @click="approveVideo(item.id)">通过</button>
      ...
      <div v-show="currentTab === 'music'" class="tab-content">
        ...
        <div v-for="item in musicList" :key="'m-' + item.id" class="review-card">
          ...
          <div class="card-right">
            <button class="btn-approve" @click="approveMusic(item.id)">通过</button>
...
async function loadStats() {
  try {
    ...
    if (res.success) {
      ...
      // 图文pending = filter by type on client side
      stats.postPending = 0
    }
  } catch (e) {
    console.error(e)
