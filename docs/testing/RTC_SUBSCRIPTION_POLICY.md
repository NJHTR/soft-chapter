...
| 面板状态 | 群聊视频订阅 | 音频订阅 | 说明 |
|---|---|---|---|
| 已接通、正常显示 | 所有已知远端参与者 | 保持 | 当前网格布局没有引入虚假的 viewport 判断 |
| 已接通、最小化 | active speaker；没有 active speaker 时取第一个远端 | 保持 | 减少小窗状态下的远端视频下行 |
| 浏览器标签页后台 | active speaker；没有 active speaker 时取第一个远端 | 保持 | 恢复前台后重新应用正常可见集合 |
| 1 对 1 | 兼容全量模式 | 保持 | 不改变现有通话画面和黑屏修复路径 |
